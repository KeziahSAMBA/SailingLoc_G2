...
// Dépose un avis sur une location réellement terminée. L'avis reste en attente
// de modération avant d'être exposé par /api/reviews/public.
export async function createBookingReview(id_user, id_booking, { rating, comment }) {
  ...
  if (!Number.isInteger(bookingId) || bookingId <= 0) {
    throw Object.assign(new Error('Identifiant de réservation invalide.'), { status: 400 });
  }
  if (!Number.isInteger(parsedRating) || parsedRating < 1 || parsedRating > 5) {
    throw Object.assign(new Error('La note doit être comprise entre 1 et 5.'), { status: 400 });
  }
  ...
  const booking = await prisma.booking.findFirst({
    where: { id_booking: bookingId, id_user, deleted_at: null },
    select: {
      id_booking: true,
      status: true,
      end_date: true,
  ...
  if (booking.status !== 'confirmed') {
    throw Object.assign(new Error('Seule une location confirmée peut être évaluée.'), {
      status: 409,
    });
  }
