  ...
  return (
    <ul aria-hidden className="grid gap-3 xl:grid-cols-2">
      {Array.from({ length: count }).map((_, i) => (
        <li key={i}>
          <div
            className={`${height} animate-pulse rounded-2xl border border-white/10 bg-white/5 p-4 motion-reduce:animate-none`}
          >
            <div className="flex items-start gap-4">
              {withIcon && <div className="h-11 w-11 shrink-0 rounded-xl bg-white/10" />}
