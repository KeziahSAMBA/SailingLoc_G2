  ...
  return (
    <section aria-labelledby="admin-cron-runs-title">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 id="admin-cron-runs-title" className="text-2xl font-bold text-on-dark">
            {t('adminCronRuns.title')}
          </h1>
          <p className="mt-1 text-sm text-on-dark/70">{t('adminCronRuns.subtitle')}</p>
        </div>
        <button
          type="button"
          onClick={() => load()}
          className={`flex items-center gap-2 rounded-full bg-surface/10 px-4 py-2 text-sm font-medium text-on-dark/80 transition hover:bg-surface/20 hover:text-on-dark ${FOCUS_RING}`}
        >
      ...
      <div
        aria-live="polite"
        className="mb-6 rounded-2xl border border-glass/20 bg-surface/10 p-4 backdrop-blur-xl"
      >
        <h2 className="text-xs font-semibold uppercase tracking-wide text-on-dark/60">
          {t('adminCronRuns.nowRunning')}
        </h2>
        {running.length === 0 ? (
          <p className="mt-2 text-sm text-on-dark/70">{t('adminCronRuns.nothingRunning')}</p>
        ) : (
          <ul className="mt-3 space-y-2">
            {running.map((run) => (
              <li
                key={run.id_run}
                className="flex flex-wrap items-center gap-3 rounded-xl bg-surface/5 px-3 py-2"
              >
                <span
                  aria-hidden="true"
                  className="h-2.5 w-2.5 shrink-0 animate-pulse rounded-full bg-action-bright"
                />
                <span className="font-medium text-on-dark">{jobLabel(run.key)}</span>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          <label htmlFor="run-job" className="mb-1 block text-xs font-medium text-on-dark/70">
        ...
        <div>
          <label htmlFor="run-status" className="mb-1 block text-xs font-medium text-on-dark/70">
          ...
          >
            ...
            {['running', 'success', 'failed', 'skipped'].map((status) => (
              <option key={status} value={status} className="text-content">
                {statusLabel(status)}
              </option>
            ))}
          </select>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-glass/15">
            {loading || runs.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-3 py-8 text-center text-on-dark/70">
                  {loading ? t('adminCronRuns.loading') : t('adminCronRuns.empty')}
                </td>
              </tr>
            ) : (
              runs.map((run) => (
                <tr key={run.id_run} className="text-on-dark/90 transition hover:bg-surface/5">
                  ...
                  <td className="px-3 py-3">
                    ...
                    {run.dry_run && (
                      <div className="text-xs text-warning-soft">{t('adminCron.dryRun')}</div>
                    )}
                  </td>
                  <td className="px-3 py-3">
                    <span
                  ...
                  <td className="px-3 py-3 text-on-dark/70">{triggerLabel(run.trigger)}</td>
                  <td className="px-3 py-3 text-right tabular-nums">{run.affected}</td>
                  <td className="px-3 py-3 text-right tabular-nums text-on-dark/70">
          ...
          runs.map((run) => (
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                ...
                <time dateTime={run.started_at} className="text-xs text-on-dark/60">
                  {formatDate(run.started_at, DATE_OPTS)}
                </time>
              </div>
              <p className="mt-2 text-sm font-medium text-on-dark/90">{jobLabel(run.key)}</p>
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <h2 id="cron-run-detail-title" className="text-lg font-semibold text-on-dark">
            ...
            {selected.error && (
              <>
                ...
                <p className="mt-1 rounded-lg bg-danger-base/10 p-3 text-sm text-danger-pale">
                  {selected.error}
                </p>
              </>
            )}
                  ...
                  <>
                    <ul className="mt-2 flex max-h-40 flex-wrap gap-1.5 overflow-y-auto">
                      {selected.result.targets.map((id) => (
                        <li
                          key={id}
                          className="rounded-md bg-surface/10 px-2 py-0.5 font-mono text-xs text-on-dark/80"
                        >
                          #{id}
                        </li>
                      ))}
