export function HeaderDropdown({ children, width = 'w-48' }) {
  return (
    <div
      className={`absolute right-0 mt-2 ${width} overflow-hidden rounded-lg bg-surface shadow-xl ring-1 ring-slate-200`}
    >
      {children}
    </div>
  );
...
export function HeaderDropdownItem({ onClick, icon, children, borderTop = true, danger = false }) {
  return (
    <button
      ...
      } ${danger ? 'text-red-600 hover:bg-red-50' : 'text-slate-700 hover:bg-page'}`}
    >
      {icon}
      {children}
    </button>
