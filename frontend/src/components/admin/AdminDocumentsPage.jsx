...
function AdminDocumentsPage() {
  ...
  return (
    <section>
      <h1 className="text-2xl font-bold text-on-dark">{t('adminDocuments.title')}</h1>
      <p className="mt-1 text-sm text-on-dark/70">{t('adminDocuments.subtitle')}</p>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-glass/15">
            {loading ? (
              ...
              pageDocuments.map((d) => (
                <tr key={d.id_document} className="text-on-dark/90">
                  ...
                  <td className="px-4 py-3 text-on-dark/70">{fmtDate(d.upload_date)}</td>
                  <td className="px-4 py-3">
                    <span
        ...
        {loading || documents.length === 0 ? (
          ...
          pageDocuments.map((d) => (
            ...
            >
              ...
              <p className="mt-1 break-all text-xs text-on-dark/60">
                {d.user?.email} {d.user ? `· ${roleLabel(d.user.role)}` : ''}
              </p>

              <p className="mt-2 break-words text-sm text-on-dark/90">{typeLabel(d.type)}</p>
              <p className="text-xs text-on-dark/60">{fmtDate(d.upload_date)}</p>
